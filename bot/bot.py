...
def main():
    ...
    while True:
        try:
            ...
            try:
                j.run_repeating(payment_checker, interval=60, first=60)
            except: pass
            try:
                j.run_repeating(scheduled_tasks, interval=3600, first=3600)
            except: pass
            #j.run_repeating(relatorio_task, interval=43200, first=43200)
            ...
            updater.start_polling(poll_interval=5.0, timeout=100, clean=None, bootstrap_retries=1000, read_latency=10.0, allowed_updates=None, drop_pending_updates=True)
            break
